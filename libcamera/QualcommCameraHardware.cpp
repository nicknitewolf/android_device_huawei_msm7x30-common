...
static FPSRange FpsRangesSupported[] = {
    FPSRange(MINIMUM_FPS*1000,MAXIMUM_FPS*1000)
};
...
static const str_map focus_modes[] = {
    ...
    { QCameraParameters::FOCUS_MODE_CONTINUOUS_VIDEO, AF_MODE_CAF_VID }
};
...
//  {QCameraParameters::PIXEL_FORMAT_YV12, CAMERA_YUV_420_YV12 }
};
...
void QualcommCameraHardware::initDefaultParameters()
{
    ...
    mParameters.setPictureFormat(QCameraParameters::PIXEL_FORMAT_JPEG); // informative
    ...
    mParameters.set(QCameraParameters::KEY_POWER_MODE_SUPPORTED, "false");
    ...
    if (zoomSupported) {
         ...
         */
        mParameters.set(QCameraParameters::KEY_MAX_ZOOM,mMaxZoom-1);
    ...
    mParameters.set(QCameraParameters::KEY_ZOOM_SUPPORTED, "true");
    mParameters.set(QCameraParameters::KEY_ZOOM, 0);
    ...
    mParameters.setFloat(QCameraParameters::KEY_HORIZONTAL_VIEW_ANGLE, horizontalViewAngle);
    mCfgControl.mm_camera_get_parm(CAMERA_PARM_VERTICAL_VIEW_ANGLE, &verticalViewAngle);
    mParameters.setFloat(QCameraParameters::KEY_VERTICAL_VIEW_ANGLE, verticalViewAngle);
...
void QualcommCameraHardware::setExifInfo(void)
{
    ...
    // set timestamp
    const char *date_str = mParameters.get(QCameraParameters::KEY_EXIF_DATETIME);
    if (date_str) {
...
bool QualcommCameraHardware::initImageEncodeParameters(int size)
{
    ...
    int jpeg_quality = mParameters.getInt(QCameraParameters::KEY_JPEG_QUALITY);
    bool ret;
    if (jpeg_quality >= 0) {
    ...
    int thumbnail_quality = mParameters.getInt(QCameraParameters::KEY_JPEG_THUMBNAIL_QUALITY);
...
bool QualcommCameraHardware::initPreview()
{
    ...
    const char *recordSize = mParameters.get(QCameraParameters::KEY_VIDEO_SIZE);
...
void QualcommCameraHardware::runSmoothzoomThread(void *data) {
    ...
    ALOGV("runSmoothzoomThread: Current zoom %d - "
          "Target %d", mParameters.getInt(QCameraParameters::KEY_ZOOM), mTargetSmoothZoom);
    int current_zoom = mParameters.getInt(QCameraParameters::KEY_ZOOM);
    ...
    while (true) {  // Thread loop
        ...
        // update zoom
        p.set(QCameraParameters::KEY_ZOOM, i);
        setZoom(p);
        if (!mPreviewStopping) {
...
status_t QualcommCameraHardware::setExposureCompensation(const QCameraParameters & params)
{
    ...
    if (EXPOSURE_COMPENSATION_MINIMUM_NUMERATOR <= numerator &&
        ...
        mParameters.set(QCameraParameters::KEY_EXPOSURE_COMPENSATION, numerator);
...
status_t QualcommCameraHardware::setSkinToneEnhancement(const QCameraParameters& params)
{
    ...
    int skinToneValue = params.getInt(QCameraParameters::KEY_SKIN_TONE_ENHANCEMENT);
...
status_t QualcommCameraHardware::setZoom(const QCameraParameters& params)
{
    ...
    int32_t zoom_level = params.getInt(QCameraParameters::KEY_ZOOM);
    if (zoom_level >= 0 && zoom_level <= mMaxZoom-1) {
        mParameters.set(QCameraParameters::KEY_ZOOM, zoom_level);
...
status_t QualcommCameraHardware::setMeteringAreas(const QCameraParameters& params)
{
    ...
    if (str == NULL || (strcmp(str, "0") == 0)) {
        ...
        if ((strcmp("(-2000,-2000,-2000,-2000,0)", str) == 0) ||
            ...
            mParameters.set(QCameraParameters::KEY_METERING_AREAS, "");
            return NO_ERROR;
        }
...
status_t QualcommCameraHardware::setFocusAreas(const QCameraParameters& params)
{
    ...
    if (str == NULL || strcmp(str, "0") == 0) {
        ...
        if (strcmp("(-2000,-2000,-2000,-2000,0)", str) == 0 ||
            strcmp("(0,0,0,0,0)", str) == 0) {
            mParameters.set(QCameraParameters::KEY_FOCUS_AREAS, "");
            return NO_ERROR;
        }
